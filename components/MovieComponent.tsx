...
function MovieComponent(props) {
  ...
  useMutation({
    mutationFn: (movie: any) => axios.post('/api/addtocart',{moviesIDs :[movie.id.toString()]}, {headers: {"authorization" : localStorage.getItem("token") }}),
    onSuccess: () =>{
      setData((data) => {
  ...
  return (
    ...
    <img
     className="h-96 w-60 object-fill rounded-md"
      src={movie.poster_path ? movie.poster_path : 'https://www.altavod.com/assets/images/poster-placeholder.png'}
      alt=""
    />
    ...
    <div className="absolute bottom-0 bg-red w-60 h-10 rounded-br-md rounded-bl-md p-4 flex justify-between">
      ...
      <button className='' onClick={() => handleAddToCartClick(movie)}><i className="fa-solid fa-cart-plus"></i></button>
    </div>
  </div>
  )
}
