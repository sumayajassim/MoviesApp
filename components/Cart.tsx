...
export default function Modal(props) {
  ...
  useMutation({
    mutationFn: (movieID: any) => axios.post('/api/removefromcart',{moviesIDs :[movieID.toString()]}, {headers: {"Authorization" : localStorage.getItem("token") }}),
    onSuccess: (res, movieID) =>{
      const newCartItems = data.cart.filter((movie:any) => movie.id !== movieID)
  ...
  const cartItems = data.cart?.map((item) => (
    <li className="border-t-1 border-b-1 list-none p-4 flex items-center">
      ...
      <div className="flex flex-col ml-4 items">
        ...
        <div className="">
          <i className="fa-solid fa-trash text-red-500 text-sm btn pl-0" onClick={() => handleRemoveFromCart(item.id)}></i>
            ...
            <div className="relative w-auto my-6 mx-auto max-w-4xl ">
              ...
              <div className="border-0 rounded-lg shadow-lg  relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="relative p-0 flex-auto overflow-auto max-h-[50vh] min-h-[50vh] max-w-4xl min-w-[60%]">
                  {cartItems.length > 0 ? cartItems : (<div className="p-5 w-72 h-44">Your cart is empty</div>)}
                </div>
                {/*footer*/}
                {cartItems.length > 0  && <div className="flex flex-col items-end justify-center p-6 border-solid rounded-b">
